...
if ([string]::IsNullOrWhiteSpace($AntigravityDir)) {
    ...
    foreach ($c in $candidates) {
        if (Test-Path (Join-Path $c "Antigravity IDE.exe")) {
            $AntigravityDir = $c
            break
        }
...
if ([string]::IsNullOrWhiteSpace($AntigravityDir) -or (-not (Test-Path (Join-Path $AntigravityDir "Antigravity IDE.exe")) -and -not (Test-Path (Join-Path $AntigravityDir "Antigravity.exe")))) {
    Write-Err "Antigravity.exe or Antigravity IDE.exe not found. Pass -AntigravityDir 'C:\path\to\Antigravity'"
    exit 1
}

Write-Info "Install target: $AntigravityDir"

# Resolve language server binary directory
$binDir = ""
$binCandidates = @(
    (Join-Path $AntigravityDir "resources\app\extensions\antigravity\bin"),
    (Join-Path $AntigravityDir "resources\bin")
)
...
if ([string]::IsNullOrWhiteSpace($binDir)) {
    Write-Err "Language server binary directory not found. Checked: resources\app\extensions\antigravity\bin, resources\bin"
    exit 1
}
...
# Determine launcher target executable name
$exeName = "Antigravity.exe"
if (Test-Path (Join-Path $AntigravityDir "Antigravity IDE.exe")) {
...
if exist "%~dp0config.proxy.json" (
  ...
  if exist "%~dp0resources\app\extensions\antigravity\bin\" copy /Y "%~dp0config.proxy.json" "%~dp0resources\app\extensions\antigravity\bin\config.json" >nul
)
